...
def order_created(order_id):
    ...
    mail_sent = send_mail(subject, message, 'gmail с которого будут отправляться сообщения', [order.email], fail_silently=False)
    print('OK ', order_id)
    return mail_sent
...
def order_push(order_id):
    ...
    mail_sent = send_mail(subject, message, 'gmail с которого будут отправляться сообщения', ['Адресс на которй будут приходить сообщения'], fail_silently=False)
    print('OK2 ', order_id)
    return mail_sent
